...
To make starting a node for the ZentroscanNetwork as quick as possible, the _quickstart_ script can be used.

1. Download the script.
2. Download one of the example `.env` files located at the [examples folder](https://github.com/Zentroscanio/Zentroscan-network/tree/master/scripts/examples).
...
$ wget -O .env https://raw.githubusercontent.com/Zentroscan Coin/master/scripts/examples/.env.<ROLE>.example
$ ./quickstart.sh
```
...
To run the parity client for the ZentroscanNetwork you first have to pull the image from [DockerHub](https://hub.docker.com/r/Zentroscannet/node).
...
```text
$ docker pull Zentroscannet/node
$ docker run Zentroscannet/node --help
...
#   The key-set is expected to to be placed in the custom configuration folder under 'keys/ZentroscanNetwork/'
...
$ docker run \
    --detach \
    --name Zentroscannet \
    --volume $(pwd)/Zentroscannet/database:/data \
    --volume $(pwd)/Zentroscannet/config:/config/custom \
    -p 30303:30300/tcp \
    -p 30303:30300/udp \
    -p 8545:8545 \
    -p 8546:8546 \
...
Inside a directory for the keys with another sub-directory for the ZentroscanNetwork chain is used by _Parity_.
...
If you have no account already or want to create a new one for this purpose checkout [this section](https://github.com/Zentroscanio/Zentroscan-network/blob/master/README.md#create-new-account).
...
```text
$ mkdir -p ./config/keys/ZentroscanNetwork
$ cp /path/to/my/key ./config/keys/ZentroscanNetwork/
...
One is the [validator-app](https://github.com/Zentroscanio/Zentroscan-network/tree/master/app)
...
$ docker run \
    ...
    --volume $(pwd)/Zentroscannet/config:/config/custom \
    --restart=on-failure \
    Zentroscannet/validator-app
```

Second one is the [bridge-oracle](https://github.com/Zentroscanio/bridge-oracle)

```text
$ wget -O docker-compose.yml https://raw.githubusercontent.com/Zentroscanio/bridge-oracle/master/docker-compose.keystore.yml
...
In relation to the instructions for the [validator](https://github.com/Zentroscanio/Zentroscan-network/blob/master/README.md#validator) role, we use the folder called `config` to bind as _Docker_ volume to _Parity_.

Afterwards the key will be placed there and the first steps of these instructions can be skipped.

```text
$ mkdir ./config
$ docker run -ti -v $(pwd)/config/:/config/custom Zentroscannet/node --parity-args account new
```
...
If you want to run a node to be used by the [blockscout explorer](https://github.com/Zentroscanio/blockscout/tree/Zentroscan) run the following command:
...
All roles should also run a [Ethereum Network Intelligence API](https://github.com/Zentroscanio/eth-net-intelligence-api) app as well, in order to connect themselves as part of the network and be viewed by the [health](https://status.Zentroscan.com/) service

```text
$ docker run \
    --detach \
    --name Zentroscannetstat \
    --net=container:Zentroscannet \
    --restart=on-failure \
    Zentroscannet/netstat \
